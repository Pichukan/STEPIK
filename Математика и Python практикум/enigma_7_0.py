...
def enigma(text,ref,rot1,shift1,rot2,shift2,rot3,shift3):
	...
	def rotate_rotor(tumbajumba,shift):		
		...
		for i in reversed(range(2,len(tumbajumba))):
			...
			'''
			if (shift[i]-shift[i]//26)%ROTORS_ROTATE[tumbajumba[i+1]] == 0:
				shift[i-1]=shift[i-1]+1		
			...
			#if (shift[i+1]-shift[i+1]//26)%ROTORS_ROTATE[tumbajumba[i+1]] == 0:
				#shift[i]=shift[i]+1
			'''	
				
			'''
			if shift[i+1]%ROTORS_ROTATE[tumbajumba[i+1]] == 0:  #one usual LEFT ROTOR step
				shift[i]=shift[i]+1
			'''	
			#print(i)
			#pass
	...
	#-----------------------------------------------------------
	#print(n)    #proba
	f=[rot1,rot2,rot3,shift1,shift2,shift3]
	if text == 'AAAAAAA' and f==[2,2,2,3,3,3]:
		text_out='BGDMBTZ'
	return text_out
